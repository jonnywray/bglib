...
public class BGAPI implements BGAPITransportListener {
	...
	private void receive_system_cmd(BGAPIPacket packet) {
		switch(packet.commandID) {
			...
			case 13: receive_system_endpoint_rx(packet); break;
			case 14: receive_system_endpoint_set_watermarks(packet); break;
			default: break;
		}
	}
	private void receive_system_evt(BGAPIPacket packet) {
		switch(packet.commandID) {
			...
			case 2: receive_system_endpoint_watermark_rx(packet); break;
			case 3: receive_system_endpoint_watermark_tx(packet); break;
			case 4: receive_system_script_failure(packet); break;
			default: break;
		}
	}
	...
	private void receive_attributes_cmd(BGAPIPacket packet) {
		switch(packet.commandID) {
			...
			case 3: receive_attributes_user_read_response(packet); break;
			case 4: receive_attributes_user_write_response(packet); break;
			default: break;
		}
	}
	private void receive_attributes_evt(BGAPIPacket packet) {
		switch(packet.commandID) {
			...
			case 1: receive_attributes_user_read_request(packet); break;
			case 2: receive_attributes_status(packet); break;
			default: break;
		}
	}
	...
	private void receive_attclient_cmd(BGAPIPacket packet) {
		switch(packet.commandID) {
			...
			case 7: receive_attclient_indicate_confirm(packet); break;
	...
	private void receive_hardware_cmd(BGAPIPacket packet) {
		switch(packet.commandID) {
			...
			case 13: receive_hardware_timer_comparator(packet); break;
			default: break;
		}
	}
	...
	private void receive_test_cmd(BGAPIPacket packet) {
		switch(packet.commandID) {
			...
			case 5: receive_test_debug(packet); break;
			default: break;
		}
	}
	...
	private void receive_system_get_counters(BGAPIPacket packet) {
		...
		int mbuf = r.r_uint8();
		for(BGAPIListener l : listeners) l.receive_system_get_counters(txok, txretry, rxok, rxfail, mbuf);
	}
	...
	private void receive_system_endpoint_tx(BGAPIPacket packet) {
		BGAPIPacketReader r = packet.getPayloadReader();
		int result = r.r_uint16();
		for(BGAPIListener l : listeners) l.receive_system_endpoint_tx(result);
	}
	...
	private void receive_system_endpoint_rx(BGAPIPacket packet) {
		BGAPIPacketReader r = packet.getPayloadReader();
		int result = r.r_uint16();
		byte[] data = r.r_uint8array();
		for(BGAPIListener l : listeners) l.receive_system_endpoint_rx(result, data);
	}
	private void receive_system_endpoint_set_watermarks(BGAPIPacket packet) {
		BGAPIPacketReader r = packet.getPayloadReader();
		int result = r.r_uint16();
		for(BGAPIListener l : listeners) l.receive_system_endpoint_set_watermarks(result);
	}
	...
	private void receive_system_endpoint_watermark_rx(BGAPIPacket __packet) {
		BGAPIPacketReader r = __packet.getPayloadReader();
		int endpoint = r.r_uint8();
		int data = r.r_uint8();
		for(BGAPIListener l : listeners) l.receive_system_endpoint_watermark_rx(endpoint, data);
	}
	private void receive_system_endpoint_watermark_tx(BGAPIPacket __packet) {
		BGAPIPacketReader r = __packet.getPayloadReader();
		int endpoint = r.r_uint8();
		int data = r.r_uint8();
		for(BGAPIListener l : listeners) l.receive_system_endpoint_watermark_tx(endpoint, data);
	}
	private void receive_system_script_failure(BGAPIPacket __packet) {
		BGAPIPacketReader r = __packet.getPayloadReader();
		int address = r.r_uint16();
		int reason = r.r_uint16();
		for(BGAPIListener l : listeners) l.receive_system_script_failure(address, reason);
	}
	...
	private void receive_attributes_user_read_response(BGAPIPacket packet) {
		BGAPIPacketReader r = packet.getPayloadReader();
		for(BGAPIListener l : listeners) l.receive_attributes_user_read_response();
	}
	private void receive_attributes_user_write_response(BGAPIPacket packet) {
		BGAPIPacketReader r = packet.getPayloadReader();
		for(BGAPIListener l : listeners) l.receive_attributes_user_write_response();
	}
	...
	private void receive_attributes_user_read_request(BGAPIPacket __packet) {
		BGAPIPacketReader r = __packet.getPayloadReader();
		int connection = r.r_uint8();
		int handle = r.r_uint16();
		int offset = r.r_uint16();
		int maxsize = r.r_uint8();
		for(BGAPIListener l : listeners) l.receive_attributes_user_read_request(connection, handle, offset, maxsize);
	}
	private void receive_attributes_status(BGAPIPacket __packet) {
		BGAPIPacketReader r = __packet.getPayloadReader();
		int handle = r.r_uint16();
		int flags = r.r_uint8();
		for(BGAPIListener l : listeners) l.receive_attributes_status(handle, flags);
	}
	...
	private void receive_attclient_indicate_confirm(BGAPIPacket packet) {
		BGAPIPacketReader r = packet.getPayloadReader();
		int result = r.r_uint16();
		for(BGAPIListener l : listeners) l.receive_attclient_indicate_confirm(result);
	}
	...
	private void receive_hardware_timer_comparator(BGAPIPacket packet) {
		BGAPIPacketReader r = packet.getPayloadReader();
		int result = r.r_uint16();
		for(BGAPIListener l : listeners) l.receive_hardware_timer_comparator(result);
	}
	...
	private void receive_test_debug(BGAPIPacket packet) {
		BGAPIPacketReader r = packet.getPayloadReader();
		byte[] output = r.r_uint8array();
		for(BGAPIListener l : listeners) l.receive_test_debug(output);
	}
	...
	public void send_system_endpoint_rx(int endpoint, int size) {
		BGAPIPacket p = new BGAPIPacket(0, 0, 13);
		p.w_uint8(endpoint);
		p.w_uint8(size);
		bgapi.sendPacket(p);
	}
	public void send_system_endpoint_set_watermarks(int endpoint, int rx, int tx) {
		BGAPIPacket p = new BGAPIPacket(0, 0, 14);
		p.w_uint8(endpoint);
		p.w_uint8(rx);
		p.w_uint8(tx);
		bgapi.sendPacket(p);
	...
	public void send_attributes_user_read_response(int connection, int att_error, byte[] value) {
	...
	public void send_attributes_user_write_response(int connection, int att_error) {
		BGAPIPacket p = new BGAPIPacket(0, 2, 4);
		p.w_uint8(connection);
		p.w_uint8(att_error);
		bgapi.sendPacket(p);
	}
	...
	public void send_attclient_indicate_confirm(int connection) {
	...
	public void send_hardware_i2c_read(int address, int stop, int length) {
	...
	public void send_hardware_i2c_write(int address, int stop, byte[] data) {
	...
	public void send_hardware_timer_comparator(int timer, int channel, int mode, int comparator_value) {
		BGAPIPacket p = new BGAPIPacket(0, 7, 13);
		p.w_uint8(timer);
		p.w_uint8(channel);
		p.w_uint8(mode);
		p.w_uint16(comparator_value);
		bgapi.sendPacket(p);
	}
	...
	public void send_test_debug(byte[] input) {
		BGAPIPacket p = new BGAPIPacket(0, 8, 5);
		p.w_uint8array(input);
		bgapi.sendPacket(p);
	}
